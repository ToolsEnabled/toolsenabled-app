...
const tracked=['shell/main.cjs','shell/agent-command-surface.cjs','shell/agent-host.cjs','shell/standalone-switch-coordinator.cjs',
 ...
 'tools/test/agent-command-surface.test.mjs','tools/test/t545-standalone-image-binding-review.test.mjs','tools/test/t545-standalone-start-boundary-review.test.mjs']
...
async function setup(t){
 ...
 let starts=0,sends=0,selectedWorkspace=root
 const launchOptions=[]
 ...
 const deps=fakeDeps({parseAgentStart:helpers.parseAgentStart,agentPayload:helpers.agentPayload,
  currentAgentHost:()=>host,getAgentHost:()=>host,chosenWorkspaceCwd:()=>selectedWorkspace,ensureWorkspaceRoot:()=>root,
 ...
 return {root:scratch,host,store,capture,surface,principal,context,sourceId,standalone,computerId,deps,
  launchOptions,selectWorkspace:value=>{selectedWorkspace=value},starts:()=>starts,sends:()=>sends,queue:r=>surface.run('agent:image-queue',{conversationId:standalone.id,sessionId:sourceId,ownerContext:context,...r},principal)}
}
test('renderer cwd remains refused before engine start',async t=>{
 const f=await setup(t)
 await assert.rejects(f.surface.run('agent:start',{sessionId:randomUUID(),tier:'luna',cwd:root},f.principal),{code:'MC_AGENT_CWD_NOT_YOURS'})
 assert.equal(f.starts(),1)
 assert.equal(f.host.sessionIsOnTree(f.sourceId),false)
})
test('real parser accepts omitted effort but explicitly rejects undefined effort',()=>{
 assert.equal(helpers.parseAgentStart({sessionId:randomUUID(),tier:'luna'}).tier,'luna')
 assert.throws(()=>helpers.parseAgentStart({sessionId:randomUUID(),tier:'luna',effort:undefined}),{code:'MC_AGENT_INVALID_PAYLOAD'})
})
test('replacement retains source cwd after chosen workspace changes',async t=>{
 const f=await setup(t)
 f.selectWorkspace(f.root)
 const operationId=randomUUID()
 const prepared=await f.surface.run('agent:switch',{operation:'prepare',sessionId:f.sourceId,operationId,tier:'luna',account:{mode:'keep'}},f.principal)
 assert.equal(prepared.phase,'prepared')
 assert.equal(f.starts(),2)
 assert.equal(f.launchOptions[1].cwd,f.launchOptions[0].cwd)
 assert.notEqual(f.launchOptions[1].cwd,f.root)
 const committed=await f.surface.run('agent:switch',{operation:'commit',sessionId:f.sourceId,operationId},f.principal)
 assert.equal(committed.applied,true)
 assert.deepEqual(f.capture.bindingFor(committed.sessionId),{computerId:f.computerId,nodeId:f.standalone.id})
})
