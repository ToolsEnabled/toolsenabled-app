'use strict'
const { app, BrowserWindow, session } = require('electron')
const fs = require('node:fs'); const path = require('node:path'); const { fileURLToPath } = require('node:url')
const data = process.argv[2]
if (!data || !path.isAbsolute(data) || fs.realpathSync.native(data) !== data) throw Error('Real isolated output required')
for (const key of ['userData', 'sessionData', 'logs', 'crashDumps']) { const dir = path.join(data, key); fs.mkdirSync(dir); app.setPath(key, dir) }
app.disableHardwareAcceleration()
const report = { cases: [], pageErrors: [], deniedRequests: [] }; let win
const save = () => fs.writeFileSync(path.join(data, 'observations.json'), JSON.stringify(report, null, 2) + '\n')
app.on('window-all-closed', () => {})
app.whenReady().then(async () => {
  session.defaultSession.webRequest.onBeforeRequest((details, callback) => { const allowed = details.url.startsWith('data:') || (details.url.startsWith('file:') && path.dirname(fileURLToPath(details.url)) === data); if (!allowed) report.deniedRequests.push(details.url); callback({ cancel: !allowed }) })
  win = new BrowserWindow({ show: false, width: 1400, height: 832, useContentSize: true, webPreferences: { sandbox: true, contextIsolation: true, nodeIntegration: false, offscreen: true, backgroundThrottling: false } })
  win.webContents.setWindowOpenHandler(() => ({ action: 'deny' }))
  win.webContents.on('console-message', d => { if (d.level === 'error') report.pageErrors.push(d.message) })
  await win.loadFile(path.join(data, 'index.html'))
  /* The window is hidden and offscreen, so its page never has focus and no
     :focus or :focus-visible rule could apply to the field the fixture focuses.
     Focus emulation gives the page focus the way a shown window has it. */
  win.webContents.debugger.attach('1.3'); await win.webContents.debugger.sendCommand('Emulation.setFocusEmulationEnabled', { enabled: true })
  for (const theme of ['white', 'black']) for (const mode of ['create', 'sign-in']) for (const size of [[1400, 832], [320, 832], [390, 832]]) {
    win.setContentSize(size[0], size[1]); await new Promise(resolve => setTimeout(resolve, 0)); await win.webContents.executeJavaScript(`setupAccountFieldsFixture.render(${JSON.stringify({ mode, theme })})`, true); await win.webContents.executeJavaScript('new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve)))', true)
    const observed = await win.webContents.executeJavaScript('setupAccountFieldsFixture.measure()', true)
    await win.webContents.executeJavaScript(`setupAccountFieldsFixture.render(${JSON.stringify({ mode, theme, busy: true })})`, true); await win.webContents.executeJavaScript('new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve)))', true); const busy = await win.webContents.executeJavaScript('setupAccountFieldsFixture.measure()', true)
    report.cases.push({ theme, mode, viewport: size, observed, busy }); fs.writeFileSync(path.join(data, `fields-${theme}-${mode}-${size[0]}.png`), (await win.webContents.capturePage()).toPNG())
  }
  win.webContents.debugger.detach(); report.visible = win.isVisible(); win.destroy(); report.destroyed = win.isDestroyed(); save(); app.exit(0)
}).catch(error => { report.failure = { message: error.message, stack: error.stack }; save(); if (win && !win.isDestroyed()) win.destroy(); app.exit(1) })
