// Offline reconstruction of the stage ledger, chosen program and native
...
export async function verifyWorkflowEvidence(directory) {
  ...
  return { status: 'verified', projectSha256: project.sha256, runtimePins: RUNTIME_FILES.length, nativeExecutions: report.runs.length,
    workflowStages: events.filter(event => event.type === 'workflow-started').length, rejectedPrograms: report.runs.filter(row => !row.passed).length, retainedFiles: Object.keys(hashes).length, reportFiles: Object.keys(files).length, referenceSha256: reference.sha256 }
}
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) console.log(JSON.stringify(await verifyWorkflowEvidence(resolve(process.argv[2])), null, 2))
