...
const EVENT_KINDS = Object.freeze(['file', 'edit', 'remove', 'approve', 'decline', 'drift-observed', 'complete', 'answer', 'record', 'supersede', 'resolve']);
const BOM = '\uFEFF';
...
const TASK_COMPLETABLE_STATUSES = Object.freeze(new Set(['open', 'recurring']));
...
// never by a person clicking. decidePurchase alone accepts it, alongside the
// person, and journals whichever one actually decided -- so a setting-approved
// purchase is never misrecorded as an owner click. Every other gate (R's
// edit/remove/decide, A's answer/decline/remove, P's own remove) stays
// assertPerson-only; this is not a general widening of who "the person" is.
...
function nextRootNumber(options = {}) {
  ...
  const all = readAll({ includeRemoved: true, includeProposed: true, ...options });
  return highestRootNumber(all.records, readChain(historyFile)) + 1;
}
...
// src/lib/agent-digest/lock.js -- and ledger-gate-writer, ledger-archive and
...
/* THE CORE THE CHAIN SPEAKS FOR: identity, placement, status, the words, who
   ...
   ledger-gate-writer marks them met without a chain event, and that is not a
...
function appendChainLine(historyFile, event) {
  const line = `${JSON.stringify(event)}\n`;
  if (Buffer.byteLength(line, 'utf8') > MAX_EVENT_BYTES) fail('R_LEDGER_CHAIN_APPEND_FAILED', 'a history event is too large to record.');
  ...
  for (let attempt = 0; attempt < WRITE_ATTEMPTS; attempt += 1) {
    ...
    try {
      ...
      if (!TRANSIENT_WRITE_CODES.has(error && error.code) || attempt + 1 >= WRITE_ATTEMPTS) break;
      waitSync(25 * (attempt + 1));
    }
  }
  const failure = new OwnerRequestStoreError('R_LEDGER_CHAIN_APPEND_FAILED',
    'The request was saved but its history line could not be written. Run the history check before relying on it.');
...
function transact(options, now, mutate) {
  ...
  try {
    ...
    if (chain.broken) fail('R_LEDGER_CHAIN_BROKEN', `The history breaks at line ${chain.broken.line}; it was edited in place. Check it before writing again.`);
    ...
    for (const event of lines) appendChainLine(historyFile, event);
...
// records carry scope like R." Only file/complete/remove are agent-usable
// for T; only the owner answers/declines an A or decides/removes a P
// (assertPerson below, same gate R's own edit/remove/decide already use).
// ---------------------------------------------------------------------------
...
function answerAsk({ id, answer, actor, now } = {}, options = {}) {
  ...
  const result = transact(options, now, (document, at) => {
    ...
    const historyRow = { seq: 0, kind: 'answer', at, actor: PERSON, statusBefore: record.status, eventSha256: '' };
    const next = {
      ...entry,
      kind: record.kind || 'A',
      scope: record.scope,
    ...
    const requests = [...document.data.requests];
    requests[index] = next;
    return { requests, events: [{ kind: 'answer', actor: PERSON, record: next, historyRow }], record: next };
  });
  const record = result.outcome.record;
  return Object.freeze({ id, status: record.status, revision: result.revision, recordedAt: result.at });
}
...
function declineAsk({ id, reason, actor, now } = {}, options = {}) {
  ...
  const result = transact(options, now, (document, at) => {
    ...
    const historyRow = { seq: 0, kind: 'decline', at, actor: PERSON, statusBefore: record.status, eventSha256: '' };
    const next = {
      ...entry,
      kind: record.kind || 'A',
      scope: record.scope,
      ...
      filedBy: record.filedBy || PERSON,
      status: 'declined',
      decisions: [...record.decisions, { at, actor: PERSON, decision: 'decline', reason: text }],
      history: [...record.history, historyRow]
    };
    const requests = [...document.data.requests];
    requests[index] = next;
    return { requests, events: [{ kind: 'decline', actor: PERSON, record: next, historyRow }], record: next };
  });
  const record = result.outcome.record;
  return Object.freeze({ id, status: record.status, revision: result.revision, recordedAt: result.at });
}
...
/** The owner approves or declines one purchase: proposed -> approved/declined. */
function decidePurchase({ id, decision, reason, actor, now } = {}, options = {}) {
  assertPurchaseDecider(actor);
  assertKindId('P', id);
  if (decision !== 'approve' && decision !== 'decline') fail('R_LEDGER_DECISION_INVALID', 'the decision must be approve or decline.');
  const text = normalizeReason(reason);
  const result = transact(options, now, (document, at) => {
    ...
    // Journalled verbatim: `actor` is either PERSON or the exact standing-
    // setting string, never silently rewritten to PERSON -- a setting-decided
    // purchase must never read back as an owner click.
    const historyRow = { seq: 0, kind: decision, at, actor, statusBefore: record.status, eventSha256: '' };
    const nextPurchase = { ...(record.purchase || {}), decision: { decision, reason: text, at, actor } };
    const next = {
      ...entry,
      kind: record.kind || 'P',
      scope: record.scope,
      ...
      decisions: [...record.decisions, { at, actor, decision, reason: text }],
      history: [...record.history, historyRow]
    };
    const requests = [...document.data.requests];
    requests[index] = next;
    return { requests, events: [{ kind: decision, actor, record: next, historyRow }], record: next };
  });
  const record = result.outcome.record;
  return Object.freeze({ id, status: record.status, revision: result.revision, recordedAt: result.at });
}
...
function verifyHistory(options = {}) {
  ...
  for (const entry of document.data.requests) {
    if (!isStoreRecordId(entry)) continue;
    const record = normalizeRecord(entry);
  ...
  const missing = [...chainedIds(chain)].filter(id => !present.has(id));
