...
function fileRequest({ scope, key, words, filedBy, parentId, scopeLabel, source, proposed, why, now, kind, recurrence } = {}, options = {}) {
  if (kind === 'T') {
    const filed = store.fileTask({ scope, key, words, filedBy, scopeLabel, why, now, recurrence }, options);
