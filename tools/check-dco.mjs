...
function main() {
  ...
  // A valid but empty range is not evidence that the commits in a release were
  // certified. In particular, a misspelled/reordered release range can resolve
  // successfully while selecting nothing; rendering that as PASS turns an
  // inability to inspect any commit into a green gate.
  if (records.length === 0) {
    console.error(`DCO GATE: cannot certify empty git history for range "${range}"`);
    process.exit(1);
  }
